...
class CustomerController extends Controller
{
    ...
    function singleCustomer($id)
    {
        ...
        $transTotal = $curr . number_format($user->customer->transactions()->first([DB::raw('sum(amount) as total')])->total) ?? 0;
        $fundTotal = $curr . number_format($user->customer->transactions()->whereNotNull('wallet_funding_provider')->first([DB::raw('sum(amount) as total')])->total) ?? 0;
